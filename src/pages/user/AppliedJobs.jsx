  ...
  return (
    <div className="min-h-screen pt-16 bg-slate-50 dark:bg-[#060912]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* ── Header ── */}
        <div className="mb-8">
          <div className="text-[12px] font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-1">Applications</div>
          <h1 className="text-[26px] font-bold text-slate-900 dark:text-white mb-1">My Applications</h1>
        ...
        {apps.length === 0 ? (
          ...
          >
            ...
            <h3 className="text-[18px] font-semibold text-slate-900 dark:text-white mb-2">No applications yet</h3>
            <p className="text-[14px] text-slate-500 dark:text-slate-500 mb-6 max-w-sm">Start applying to jobs and track all your applications in one place.</p>
                ...
                >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-indigo-500/20 flex items-center justify-center overflow-hidden flex-shrink-0">
                        {app.job?.companyLogo
                          ...
                          : <span className="text-indigo-600 dark:text-indigo-400 font-bold text-[15px]">{app.job?.companyName?.[0]}</span>}
                      </div>

                      <div>
                        <Link
                          to={`/jobs/${app.job?._id}`}
                          className="text-[15px] font-semibold text-slate-800 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                        >
                          {app.job?.title}
                        </Link>
                        <p className="text-[13px] text-slate-500 dark:text-slate-500 mt-0.5">{app.job?.companyName}</p>
                        <div className="flex flex-wrap gap-3 mt-2 text-[12px] text-slate-500 dark:text-slate-600">
                          {app.job?.location && (
                  ...
                  {app.coverLetter && (
                    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-[#1e2d3d]">
                      <div className="flex items-center gap-1.5 text-[12px] text-slate-500 dark:text-slate-600 mb-2">
                        <FiFileText size={12} /> Cover Letter
                      </div>
                      <p className="text-[13px] text-slate-600 dark:text-slate-500 line-clamp-2 leading-relaxed">{app.coverLetter}</p>
                    </div>
                  )}
                </motion.div>
              )
