                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => handleToggle(job)}
                      className={`p-2 rounded-xl transition-all ${job.isActive ? 'text-green-400 hover:bg-green-500/10' : 'text-gray-500 hover:bg-gray-700'}`}
                      title={job.isActive ? 'Deactivate' : 'Activate'}>
                      {job.isActive ? <FiToggleRight size={20} /> : <FiToggleLeft size={20} />}
                    </button>
                    <Link to={`/employer/applicants/${job._id}`}
                      className="p-2 rounded-xl text-gray-400 hover:text-primary-400 hover:bg-primary-500/10 transition-all" title="View Applicants">
                      <FiUsers size={16} />
                    </Link>
                    <Link to={`/employer/jobs/edit/${job._id}`}
                      className="p-2 rounded-xl text-gray-400 hover:text-yellow-400 hover:bg-yellow-500/10 transition-all" title="Edit">
                      <FiEdit2 size={16} />
                    </Link>
                    <button onClick={() => handleDelete(job._id)}
                      className="p-2 rounded-xl text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Delete">
                      <FiTrash2 size={16} />
                    </button>
                  </div>
                </div>
